use crate::artwork_cache::{write_webp_derivative, ArtworkPaths};
...
pub fn fetch_deezer_artist_image(artist: &str) -> Option<String> {
    ...
    for candidate in result.data {
        ...
        if let Some(xl) = candidate.picture_xl {
            if !xl.is_empty() && !xl.contains("/images/artist//") {
                return Some(xl);
            }
        }
        if let Some(big) = candidate.picture_big {
            if !big.is_empty() && !big.contains("/images/artist//") {
                return Some(big);
            }
        }
    }
...
pub fn fetch_deezer_album_art(artist: &str, album: &str) -> Option<String> {
    ...
    for item in result.data {
        ...
        if let Some(xl) = item.cover_xl {
            if !xl.is_empty() && !xl.contains("/images/cover//") {
                return Some(xl);
            }
        }
    }
