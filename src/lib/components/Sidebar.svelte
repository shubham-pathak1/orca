...
<aside class="flex min-h-0 flex-col border-r border-white/8 bg-black/70 px-3 py-4 max-md:hidden">
  ...
  <nav class="flex flex-col gap-1">
    {#each navItems as item}
      <button
        class={`flex h-10 items-center gap-3 rounded-md px-3 text-left text-sm font-semibold transition ${activeView === item.id ? 'bg-white/[0.13] text-white shadow-[inset_3px_0_0_var(--accent)]' : 'text-white/62 hover:bg-white/[0.06] hover:text-white'}`}
  ...
  <div class="mt-auto flex flex-col gap-2 border-t border-white/8 pt-4">
    <button class="flex items-center gap-3 rounded-md px-3 py-2 text-left text-xs font-semibold text-white/58 hover:bg-white/[0.06] hover:text-white" disabled={isScanning} on:click={onAddFolder}>
    ...
    <button class="flex items-center gap-3 rounded-md px-3 py-2 text-left text-xs font-semibold text-white/58 hover:bg-white/[0.06] hover:text-white" disabled={isScanning} on:click={onRefresh}>
